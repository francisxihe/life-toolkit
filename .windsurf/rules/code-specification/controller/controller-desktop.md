...
export class {Module}DesktopController {
  ...
  async batchDone(idList: string[]): Promise<void> {
    await this.{module}Controller.batchDone(idList);
  }

  async batchDelete(idList: string[]): Promise<void> {
    await this.{module}Controller.batchDelete(idList);
  }
}
...
export const {module}IpcHandlers = {
  ...
  'batchDone': (idList: string[]) => new {Module}DesktopController().batchDone(idList),
  'batchDelete': (idList: string[]) => new {Module}DesktopController().batchDelete(idList),
};
```
...
// ❌ 避免的做法 - 阻塞主进程
export class TodoDesktopController {
  create(createVo: TodoVO.CreateTodoVo): TodoVO.TodoItemVo {
    // ❌ 同步处理会阻塞主进程
