...
export type ActiveProductFeature = { key: string; productStatus: string; surfaceCoverage: string };
...
export function listActiveProductFeatures(specifications: readonly ProductSpec[]): readonly ActiveProductFeature[] {
  return specifications.flatMap((specification) => [
    { key: featureKey(specification.id, 'module', specification.id), productStatus: specification.productStatus, surfaceCoverage: specification.surfaceCoverage },
    ...(specification.entities || []).flatMap((entity) => [
      { key: featureKey(specification.id, 'entity', entity.id), productStatus: entity.productStatus, surfaceCoverage: entity.surfaceCoverage },
      ...entity.fields.map((field) => ({ key: featureKey(specification.id, 'field', `${entity.id}.${field.id}`), productStatus: field.productStatus, surfaceCoverage: field.surfaceCoverage })),
    ]),
    ...(specification.views || []).map((view) => ({ key: featureKey(specification.id, 'view', view.id), productStatus: view.productStatus, surfaceCoverage: view.surfaceCoverage })),
    ...(specification.rules || []).map((rule) => ({ key: featureKey(specification.id, 'rule', rule.id), productStatus: rule.productStatus, surfaceCoverage: rule.surfaceCoverage })),
  ]);
}
