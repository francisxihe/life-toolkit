...
export class HabitRepository extends BaseRepositoryImpl<Habit, HabitFilterDto> implements HabitRepository {
  get goalRepo(): Repository<Goal> {
    return store().getRepository(Goal);
  }
  get todoRepo(): Repository<Todo> {
    return store().getRepository(Todo);
  }
  ...
  constructor() {
    ...
    super(() => store().getRepository(Habit), buildQuery);
  }
