...
import type { RouteDef, RestHandler, RestHandlerCtx } from './router';
import { ResponseHandler, defaultResponseHandler, type ResponseHandlerConfig } from './response-handlers';
...
function pushParamMeta(target: any, propertyKey: string | symbol, meta: ParamMeta) {
  const all: Record<string | symbol, ParamMeta[]> = Reflect.getMetadata(PARAMS, target) || {};
...
export function buildRoutesFromControllers(
  ...
  responseWrapperConfig?: ResponseHandlerConfig
): RouteDef[] {
  const routes: RouteDef[] = [];

  // 创建响应包装器实例
  const responseWrapper = responseWrapperConfig ? new ResponseHandler(responseWrapperConfig) : defaultResponseHandler;
  ...
  for (const C of controllers) {
    ...
    const paramsAll: Record<string | symbol, ParamMeta[]> = Reflect.getMetadata(PARAMS, proto) || {};
    ...
    for (const meta of metas) {
      ...
      const paramMetas = (paramsAll[meta.key] || []).sort((a, b) => a.index - b.index);

      const fullPath = (prefix || '') + normalizePath(meta.path);
      const handler: RestHandler = async (ctx: RestHandlerCtx) => {
        // 使用响应包装器包装 Controller 方法的执行
        return await responseWrapper.wrapControllerResult(async () => {
          const args: any[] = [];
