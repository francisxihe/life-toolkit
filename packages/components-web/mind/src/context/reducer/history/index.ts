  ...
  switch (action.type) {
    case actionTypes.SET_HISTORY: {
      ...
      if (new_history.last_snapshot) {
        if (
          new_history.undo.length > 0 &&
          new_history.undo[new_history.undo.length - 1].mindmap === action.data.mindmap
        ) {
          new_history.redo.unshift(new_history.last_snapshot);
          new_history.undo.pop();
        } else if (
          new_history.redo.length > 0 &&
          new_history.redo[0].mindmap === action.data.mindmap
        ) {
