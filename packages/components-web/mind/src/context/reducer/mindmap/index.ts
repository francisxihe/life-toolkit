...
export const defaultValue_mindmap: MindmapNode =
  JSON.parse(localStorage.getItem('mindmap') || 'null') || defaultMindmap;

export default (mindmap: MindmapNode, action: Action): MindmapNode => {
  switch (action.type) {
    ...
    case actionTypes.ADD_CHILD: {
      const new_mindmap = deepCopy(mindmap),
        node_found = findNode(new_mindmap, action.data.node_id!);
      node_found.children.push(action.data.node as MindmapNode);
      return new_mindmap;
    }
    case actionTypes.ADD_SIBLING: {
      ...
      if (action.data.parent_id) {
        ...
        const insert_index =
          node_found.children.findIndex(node => node.id === action.data.node_id) + 1;
        node_found.children.splice(insert_index, 0, action.data.node as MindmapNode);
      }
      return new_mindmap;
    }
    case actionTypes.MOVE_NODE: {
      ...
      if (action.data.is_sibling) {
        const target_index =
          parent.children.findIndex(node => node.id === action.data.target_id) + 1 ||
