      ...
      listener: event => {
        ...
        for (let i = 0; i < total; i++) {
          if (
            ...
            const child_offset = { left: children_offset_left[i], right: children_offset_right[i] };
            const child_left_of_parent =
              i === 1 ||
              (!parent_is_root && document.getElementById(node_id).dataset.tag === refer.LEFT_NODE); // i = 1，即出现双侧拖拽时，左侧发生的拖拽 child_left_of_parent 必然为 true
