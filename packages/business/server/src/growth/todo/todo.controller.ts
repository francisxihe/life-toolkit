...
export class TodoController {
  ...
  async create(
    @Body() createTodoVo: TodoVO.CreateTodoVo
  ...
  async update(
    @Param("id") id: string,
    @Body() updateVo: TodoVO.UpdateTodoVo
  ): Promise<TodoVO.TodoVo> {
    if (updateVo.repeat) {
  ...
  async page(
    @Query() query?: TodoVO.TodoPageFiltersVo
  ): Promise<TodoVO.TodoPageVo> {
  ...
  async list(
    @Query() query?: TodoVO.TodoListFiltersVo
  ...
  async doneBatch(@Body() body?: TodoVO.TodoListFiltersVo) {
    return await this.todoService.batchDone({ includeIds: body?.includeIds ?? [] });
  }
