...
export function genApiMethodWrapper(
  ...
  const { name, verb, path, paramStyle, paramTypes, returnType } =
    decoratorInfo;
  const indent = "  ";
  const lines: string[] = [];
  ...
  const httpMethod =
    verb.toLowerCase() === "delete" ? "remove" : verb.toLowerCase();
  ...
  switch (paramStyle) {
    ...
    case "id+body":
      ...
      const bodyType =
        paramTypes?.bodyType || getDefaultBodyType(name, entityCap);
    ...
    case "query":
      const queryType =
        paramTypes?.queryType || getDefaultQueryType(name, entityCap);
...
// 基于实际方法信息生成 API 方法名
function generateApiMethodName(
...
function removeObsoleteApiMethods(
  ...
  const methodsToRemove = Array.from(existingMethods).filter(
    (method) => !expectedMethods.has(method)
  );
...
export function syncApiMethods(
  ...
  let cleanedContent = removeObsoleteApiMethods(
    apiContent,
    className,
    existing,
    expectedApiMethodNames
  );
  ...
  const newMethods = missing
    .map((serverMethod) => {
      ...
      return genApiMethodWrapper(
        decoratorInfo,
        entityName,
        controllerInfo.basePath
      );
    })
    .filter(Boolean);
...
function syncApiMethodsLegacy(
  ...
  const existing = parseApiMethodNames(apiContent, className);
  const missing = serverMethods.filter((serverMethod) => {
    return !Array.from(existing).some((existingMethod: string) =>
      existingMethod.includes(serverMethod)
    );
  });
  ...
  const missingMethods = missing
    .map((methodName) => {
      ...
      return decoratorInfo
        ? genApiMethodWrapper(decoratorInfo, entityName)
        : "";
    })
    .filter(Boolean);
