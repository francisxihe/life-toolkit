...
function getApiControllerPathFromServer(serverControllerPath: string): string {
  ...
  const relativePath = path.relative(
    path.join(ROOT, "packages/business/server/src"),
    serverControllerPath
  );
  const parts = relativePath.split(path.sep);

  // 移除最后的 .controller.ts 并替换为 .ts
  const fileName = parts[parts.length - 1].replace(".controller.ts", ".ts");
  const dirName = parts[parts.length - 2]; // task, todo, etc.

  return path.join(ROOT, "packages/business/api/controller", dirName, fileName);
}
  ...
  if (fs.existsSync(apiPath)) {
    ...
    if (apiContent) {
      ...
      if (next !== apiContent) {
        ...
        if (ok)
          logLocal("Synced API controller ->", path.relative(ROOT, apiPath));
      }
    }
  } else {
...
function syncAllOnce() {
  const serverControllerPaths = fg.sync(
    path.join(SERVER_BASE, "**/*.controller.ts").replace(/\\/g, "/")
  );
...
function watchAndSync() {
  ...
  watcher.on("ready", () => {
    logLocal(
      "[dev-tools/watch-controllers] Initial scan complete. Watching for changes..."
    );
  });
...
if (require.main === module) {
  ...
  if (args.includes("--once")) {
    logLocal(
      "[dev-tools/watch-controllers] Run one-off sync (desktop + API controllers)"
    );
    syncAllOnce();
  } else {
    watchAndSync();
