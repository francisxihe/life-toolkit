...
import { RepeatMode, RepeatEndMode, MonthlyType, YearlyType, TimeUnit, WeekDay } from '../src/types';
import { calculateNextDate } from '../src/helpers/recurrence';
...
describe('计算下一个重复日期', () => {
  ...
  const testCalculateNextDate = (currentDate: dayjs.Dayjs, repeat: Partial<Repeat>): dayjs.Dayjs | null => {
    ...
    const result = calculateNextDate(currentDate, repeatObj);
