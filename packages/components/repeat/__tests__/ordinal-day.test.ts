...
import { RepeatMode, RepeatEndMode, MonthlyType, OrdinalDay, OrdinalDayType } from '../src/types';
import {
  calculateNextDate as calculateNextDateResult,
  isValidDate as isValidDateResult,
} from '../src/helpers/recurrence';
import type { Repeat } from '../src/helpers/recurrence/types';

function calculateNextDate(date: dayjs.Dayjs, repeat: Repeat): dayjs.Dayjs | null {
  const result = calculateNextDateResult(date, repeat);
  if (result.ok === false) throw new Error(result.issues.map((issue) => issue.message).join(' '));
  return result.value;
}

function isValidDate(date: dayjs.Dayjs, repeat: Repeat): boolean {
  const result = isValidDateResult(date, repeat);
  if (result.ok === false) throw new Error(result.issues.map((issue) => issue.message).join(' '));
  return result.value;
}
