 ...
 * @returns 日期是否符合重复规则
 */
export function isValidDateForRepeat(date: Dayjs, repeat: Repeat & { repeatStartDate: string }): boolean {
...
function isValidWeeklyDate(date: Dayjs, repeat: RepeatFormWeekly): boolean {
  ...
  console.log('isValidWeeklyDate - 日期:', date.format('YYYY-MM-DD'), '星期:', currentDayOfWeek, '配置weekdays:', repeat.repeatConfig.weekdays, '转换后:', weekdays, '结果:', result);
...
function isValidYearlyDate(date: Dayjs, repeat: RepeatFormYearly): boolean {
  ...
  if (yearlyType === YearlyType.MONTH) {
    ...
    // 首先检查月份是否匹配
    // if (date.month() !== monthConfig.month - 1) {
    //   return false;
    // }
