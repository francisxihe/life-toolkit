...
import type { RepeatPayload } from '../../types';
import {
  parseRepeatRule,
  type ParseResult,
  type RepeatFormCustom,
  type RepeatFormMonthly,
  type RepeatFormWeekly,
 ...
 * 根据重复配置计算下一个日期
 */
export function calculateNextDate(currentDate: Dayjs, input: RepeatPayload): ParseResult<Dayjs | null> {
  const parsed = parseRepeatRule(input);
  if (parsed.ok === false) return { ok: false, issues: parsed.issues };
  return { ok: true, value: calculateNextDateForRule(currentDate, parsed.value) };
}

function calculateNextDateForRule(currentDate: Dayjs, repeat: Repeat): Dayjs | null {
...
function getNextYearlyDate(currentDate: Dayjs, config: RepeatFormYearly['repeatConfig']): Dayjs {
  ...
  if (yearlyType === YearlyType.MONTH) {
    ...
    const nextYearTargetDate = dayjs()
      .year(currentYear + 1)
      .month(firstTargetMonth - 1)
      .startOf('month');
