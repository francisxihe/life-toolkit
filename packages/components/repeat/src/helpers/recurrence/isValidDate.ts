 ...
 * @returns 日期是否符合重复规则
 */
export function isValidDate(date: Dayjs, input: RepeatPayload): ParseResult<boolean> {
  const parsed = parseRepeatRule(input);
  if (parsed.ok === false) return { ok: false, issues: parsed.issues };
  return { ok: true, value: isValidDateForRule(date, parsed.value) };
}

function isValidDateForRule(date: Dayjs, repeat: Repeat): boolean {
