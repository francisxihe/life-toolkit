...
export class RepeatService {
  ...
  calculateNextDate(currentDate: Dayjs, repeat: Repeat): Dayjs | null {
    ...
    switch (repeat.repeatMode) {
      ...
      case RepeatMode.WEEKDAYS:
        // 周一至周五
        nextDate = this.getNextWeekday(currentDate);
        break;

      case RepeatMode.WEEKEND:
        // 周末（周六、周日）
        nextDate = this.getNextWeekend(currentDate);
        break;

      case RepeatMode.WEEKLY:
        // 每周指定星期几
        nextDate = this.getNextWeeklyDate(currentDate, repeat.repeatConfig);
        break;

      case RepeatMode.MONTHLY:
        // 每月重复
        nextDate = this.getNextMonthlyDate(currentDate, repeat.repeatConfig);
        break;

      case RepeatMode.YEARLY:
        // 每年重复
        nextDate = this.getNextYearlyDate(currentDate, repeat.repeatConfig);
        break;

      case RepeatMode.CUSTOM:
        // 自定义间隔
        nextDate = this.getNextCustomDate(currentDate, repeat.repeatConfig);
        break;
   ...
   * 获取下一个工作日（周一至周五）
   */
  private getNextWeekday(currentDate: dayjs.Dayjs): dayjs.Dayjs {
   ...
   * 获取下一个周末日期（周六、周日）
   */
  private getNextWeekend(currentDate: dayjs.Dayjs): dayjs.Dayjs {
    const nextDay = currentDate.add(1, "day");
    const dayOfWeek = nextDay.day();
  ...
  private getNextMonthlyDate(
    ...
    if (monthlyType === MonthlyType.DAY) {
      ...
      return this.findOrdinalWeekday(nextMonth, targetWeekday, ordinalWeek);
    } else if (monthlyType === MonthlyType.ORDINAL_DAY) {
      // 按序数日重复，例如每月最后一天
      const { ordinalDay, ordinalDayType } = config[MonthlyType.ORDINAL_DAY];
      const nextMonth = currentDate.add(1, "month");

      return this.findOrdinalDay(nextMonth, ordinalDay, ordinalDayType);
    }
  ...
  private getNextYearlyDate(
    ...
    if (yearlyType === YearlyType.MONTH) {
      ...
      return this.findOrdinalWeekday(
        sameMonthNextYear,
        targetWeekday,
        ordinalWeek
      );
    ...
    switch (intervalUnit) {
      ...
      case TimeUnit.WEEK:
        if (config[TimeUnit.WEEK]) {
          ...
          return this.getNextWeeklyDate(baseDate, weekConfig);
        }
      ...
      case TimeUnit.MONTH:
        if (config[TimeUnit.MONTH]) {
          ...
          return this.getNextMonthlyDate(baseDate, monthConfig);
        }
      ...
      case TimeUnit.YEAR:
        if (config[TimeUnit.YEAR]) {
          ...
          return this.getNextYearlyDate(baseDate, yearConfig);
        }
