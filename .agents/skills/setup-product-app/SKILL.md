...
3. Add `productWiki({ data: './src/wiki.ts', playwright: true })` to `vite.config.ts` (or `playwright: 'e2e'`). The plugin installs `productWikiInspect` and starts/reuses the Node service. It does **not** mount a dock. `playwright: true` POSTs `{ root, origin }` to `/api/playwright` so the tests tab can spawn `npx playwright test` from the matching consumer. To run several consumer apps at once, pass a dedicated wiki port (`productWiki({ data: './src/wiki.ts', port: 6101 })` or `PRODUCT_WIKI_PORT=6101`) so they do not share or overwrite `5101`.
