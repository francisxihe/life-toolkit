...
ECS_PATH="/root/project/life_toolkit_web"

# 清空远程目录
ssh -p 22 $ECS_USERNAME@$ECS_IP "cd $ECS_PATH && rm -rf *" || { echo "清空远程目录失败！"; exit 1; }

# 使用 rsync 同步文件
/opt/homebrew/bin/rsync -avz --include 'dist/***' --include 'ecosystem.config.js' --exclude '*' -e "ssh" "$LOCAL_DIR" "${ECS_USERNAME}@${ECS_IP}:$ECS_PATH"
