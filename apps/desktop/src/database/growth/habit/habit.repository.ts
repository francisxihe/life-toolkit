...
import { Habit as DesktopHabit, HabitStatus as DesktopHabitStatus, HabitDifficulty as DesktopHabitDifficulty } from "./habit.entity";
import { Goal as DesktopGoal } from "../goal/goal.entity";
import { Todo as DesktopTodo, TodoStatus as DesktopTodoStatus } from "../todo/todo.entity";
import {
...
export class HabitRepository implements BusinessHabitRepository {
  ...
  private buildQuery(filter: HabitListFilterDto) {
    ...
    if (filter.status) {
      qb = qb.andWhere("habit.status = :status", { status: filter.status as any });
    }

    if (filter.difficulty) {
      qb = qb.andWhere("habit.difficulty = :difficulty", { difficulty: filter.difficulty as any });
    }

    if ((filter as any).importance !== undefined) {
      qb = qb.andWhere("habit.importance = :importance", { importance: (filter as any).importance });
    }

    const keyword = (filter as any).keyword as string | undefined;
    if (keyword) {
      qb = qb.andWhere("(habit.name LIKE :kw OR habit.description LIKE :kw)", { kw: `%${keyword}%` });
    }
    ...
    if (startDateStart) {
      qb = qb.andWhere("habit.startDate >= :sds", { sds: new Date(`${startDateStart}T00:00:00`) });
    }
    if (startDateEnd) {
      qb = qb.andWhere("habit.startDate <= :sde", { sde: new Date(`${startDateEnd}T23:59:59`) });
    }

    const targetDateStart = (filter as any).targetDateStart as string | undefined;
    const targetDateEnd = (filter as any).targetDateEnd as string | undefined;
    if (targetDateStart) {
      qb = qb.andWhere("habit.targetDate >= :tds", { tds: new Date(`${targetDateStart}T00:00:00`) });
    }
    if (targetDateEnd) {
      qb = qb.andWhere("habit.targetDate <= :tde", { tde: new Date(`${targetDateEnd}T23:59:59`) });
    }
  ...
  async create(createHabitDto: CreateHabitDto): Promise<HabitDto> {
    const entity = this.repo.create({
      ...
      difficulty: (createHabitDto.difficulty as unknown as DesktopHabitDifficulty) ?? DesktopHabitDifficulty.MEDIUM,
      startDate: (createHabitDto as any).startDate ?? new Date(),
      targetDate: (createHabitDto as any).targetDate,
      status: (DesktopHabitStatus.ACTIVE as any),
  ...
  async update(id: string, updateHabitDto: UpdateHabitDto): Promise<HabitDto> {
    const entity = await this.repo.findOne({ where: { id }, relations: ["goals"] });
    if (!entity) throw new Error(`习惯不存在，ID: ${id}`);

    if ((updateHabitDto as any).name !== undefined) (entity as any).name = (updateHabitDto as any).name;
    if ((updateHabitDto as any).description !== undefined) (entity as any).description = (updateHabitDto as any).description;
    if ((updateHabitDto as any).importance !== undefined) (entity as any).importance = (updateHabitDto as any).importance;
    if ((updateHabitDto as any).tags !== undefined) (entity as any).tags = (updateHabitDto as any).tags;
    if ((updateHabitDto as any).difficulty !== undefined) (entity as any).difficulty = (updateHabitDto as any).difficulty as any;
    if ((updateHabitDto as any).startDate !== undefined) (entity as any).startDate = (updateHabitDto as any).startDate as any;
    if ((updateHabitDto as any).targetDate !== undefined) (entity as any).targetDate = (updateHabitDto as any).targetDate as any;
    if ((updateHabitDto as any).status !== undefined) (entity as any).status = (updateHabitDto as any).status as any;
    if ((updateHabitDto as any).currentStreak !== undefined) (entity as any).currentStreak = (updateHabitDto as any).currentStreak as any;
    if ((updateHabitDto as any).longestStreak !== undefined) (entity as any).longestStreak = (updateHabitDto as any).longestStreak as any;
    if ((updateHabitDto as any).completedCount !== undefined) (entity as any).completedCount = (updateHabitDto as any).completedCount as any;
  ...
  async batchUpdate(ids: string[], updateData: Partial<any>): Promise<void> {
    await this.repo.update({ id: In(ids) }, (updateData as unknown) as Partial<DesktopHabit>);
  }

  async updateStatus(id: string, status: HabitStatus, additionalData: Record<string, any> = {}): Promise<void> {
    const entity = await this.repo.findOne({ where: { id } });
    if (!entity) throw new Error(`习惯不存在，ID: ${id}`);
    Object.assign(entity, { status: status as any, ...(additionalData as any) });
  ...
  async updateStreak(id: string, increment: boolean): Promise<HabitDto> {
    ...
    if (increment) {
      ...
      (entity as any).completedCount = ((entity as any).completedCount || 0) + 1;
      if ((entity as any).currentStreak > ((entity as any).longestStreak || 0)) {
  ...
  async getHabitTodos(habitId: string): Promise<{
    ...
    const activeTodos = await this.todoRepo.findBy({ habitId, status: In([DesktopTodoStatus.TODO, DesktopTodoStatus.IN_PROGRESS]) as any });
    const completedTodos = await this.todoRepo.findBy({ habitId, status: DesktopTodoStatus.DONE as any });
    const abandonedTodos = await this.todoRepo.findBy({ habitId, status: DesktopTodoStatus.ABANDONED as any });
  ...
  async getHabitAnalyticsData(habitId: string): Promise<{
    ...
    const [completedTodos, abandonedTodos, recentTodos] = await Promise.all([
      this.todoRepo.count({ where: { habitId, status: DesktopTodoStatus.DONE as any } }),
      this.todoRepo.count({ where: { habitId, status: DesktopTodoStatus.ABANDONED as any } }),
      this.todoRepo.find({ where: { habitId }, order: { updatedAt: "DESC" as any }, take: 10 }),
    ]);
