...
import { Task as DesktopTask, TaskStatus as DesktopTaskStatus } from "./task.entity";
import {
  CreateTaskDto,
  UpdateTaskDto,
  TaskPageFilterDto,
...
export class TaskRepository /* implements import("@life-toolkit/business-server").TaskRepository */ {
  private repo: Repository<DesktopTask>;

  constructor() {
    this.repo = AppDataSource.getRepository(DesktopTask);
  }
  ...
  private toDto(entity: DesktopTask): TaskDto {
    const dto: any = {
      ...
      status: entity.status as unknown as BusinessTaskStatus,
  ...
  private buildQuery(filter: TaskListFilterDto & { excludeIds?: string[] }) {
    ...
    if (keyword) qb = qb.andWhere("(task.name LIKE :kw OR task.description LIKE :kw)", { kw: `%${keyword}%` });

    if (filter.status) qb = qb.andWhere("task.status = :status", { status: filter.status as any });
    ...
    if (startAt) qb = qb.andWhere("task.dueDate >= :startAt", { startAt: new Date(`${startAt}T00:00:00`) });
    if (endAt) qb = qb.andWhere("task.dueDate <= :endAt", { endAt: new Date(`${endAt}T23:59:59`) });
    ...
    if (doneDateStart && doneDateEnd) {
      ...
      qb = qb.andWhere("task.completedAt >= :ds", { ds: new Date(`${doneDateStart}T00:00:00`) });
    } else if (doneDateEnd) {
      qb = qb.andWhere("task.completedAt <= :de", { de: new Date(`${doneDateEnd}T23:59:59`) });
    }
  ...
  async create(createTaskDto: CreateTaskDto): Promise<BusinessTask> {
    const entity = this.repo.create({
      ...
      // status 走默认值
    } as DeepPartial<DesktopTask>);
    const saved = await this.repo.save(entity);
    return (saved as unknown) as BusinessTask;
  }
  ...
  async update(id: string, updateTaskDto: UpdateTaskDto): Promise<void> {
    ...
    if (updateTaskDto.tags !== undefined) entity.tags = updateTaskDto.tags as any;
    if ((updateTaskDto as any).endAt !== undefined) entity.dueDate = (updateTaskDto as any).endAt as any;
    if (updateTaskDto.goalId !== undefined) entity.goalId = updateTaskDto.goalId as any;
  ...
  async findById(id: string, relations?: string[]): Promise<TaskDto> {
    const entity = await this.repo.findOne({ where: { id }, relations: relations ?? ["parent", "children", "goal", "todoList"] });
    if (!entity) throw new Error(`任务不存在，ID: ${id}`);
