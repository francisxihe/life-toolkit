...
export class TaskService {
  ...
  async list(filter?: {
    ...
    if (filter.importance !== undefined) {
      queryBuilder.andWhere('task.importance = :importance', { importance: filter.importance });
    }

    if (filter.urgency !== undefined) {
      queryBuilder.andWhere('task.urgency = :urgency', { urgency: filter.urgency });
    }

    if (filter.keyword) {
      queryBuilder.andWhere('task.name LIKE :keyword', { keyword: `%${filter.keyword}%` });
    }

    if (filter.startDate) {
      queryBuilder.andWhere('task.startAt >= :startDate', { startDate: filter.startDate });
    }

    if (filter.endDate) {
      queryBuilder.andWhere('task.endAt <= :endDate', { endDate: filter.endDate });
    }

    if (filter.goalId) {
      queryBuilder.andWhere('task.goalId = :goalId', { goalId: filter.goalId });
    }

    if (filter.parentId) {
      queryBuilder.andWhere('task.parentId = :parentId', { parentId: filter.parentId });
    }
