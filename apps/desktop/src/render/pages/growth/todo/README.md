...
> 当前实现快照（2026-08-07）：桌面端只保留“当前待办、待办日历、全部待办”三个入口。当前待办使用 `DayAgendaDateProvider` 选择日期，并按过期、当日、已完成、已放弃分组；批量完成最多处理 50 条。独立本周视图和待办统计页已移除，以下旧版页面结构与统计说明仅作为历史设计参考，不是当前路由契约。
