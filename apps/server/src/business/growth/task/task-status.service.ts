...
import { OperationByIdListDto, OperationByIdListResultDto } from '@/common/operation';
import { TaskRepository } from './task.repository';
@Injectable()
export class TaskStatusService {
  constructor(private readonly taskRepository: TaskRepository) {}

  private async updateStatus(id: string, status: TaskStatus, dateField: keyof Task): Promise<void> {
    await this.taskRepository.updateStatus(id, status, dateField);
  }

  async doneBatch(params: OperationByIdListDto): Promise<OperationByIdListResultDto> {
