...
- 样式：CSS Modules + Less / Tailwind，与 `@sue/design-web-react`（前缀 `sue`）配合；页面壳层优先使用其 `Flex`（`container="full|fixed|fill"`）。`fixed` 不自动撑满交叉轴：列父加 `w-full`，行父加 `h-full`（对齐已移除的 `FlexibleContainer.Fixed`）。UI 落地细则见项目 skill「框架规范 · React UI规范」
